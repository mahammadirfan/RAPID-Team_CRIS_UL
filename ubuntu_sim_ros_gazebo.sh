...
source ~/Downloads/ubuntu_sim_common_deps_px4.sh
...
wstool update -t src
if ! rosdep install --from-paths src --ignore-src --rosdistro kinetic ; then
    ...
    rosdep install --from-paths src --ignore-src --rosdistro kinetic  --os ubuntu:xenial
fi
## Build!
catkin build
